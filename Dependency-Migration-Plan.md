...
- Tailwind: migration in progress. Version 4.3.3 uses its dedicated PostCSS plugin and explicit CSS source paths matching the old configuration. Preserve previous base border/placeholder colors; remove the empty JS configuration and redundant Autoprefixer dependency. Local lint/type checks, 612 application tests, script tests and build passed. All 14 local visual comparisons match the preceding React migration within unchanged tolerances; platform acceptance is pending.
- Electron and final nightly: pending. Electron needs separate clipboard and macOS compatibility preparation PRs before its runtime switch. Electron 44 requires macOS 13 or later; setup must explicitly install its lazily downloaded binary before CI captures its path.
