...
- Electron clipboard preparation: in progress on Electron 39. All clipboard reads/writes now have a main-process asynchronous boundary and callers await completion. Seven tests cover pending writes, native rejection, returned content and one-operation context writes. Local lint/type checks, 619 application tests, script tests and build passed; platform/native acceptance is pending.
- Electron macOS preparation, runtime switch and final nightly: pending. Electron 44 requires macOS 13 or later; setup must explicitly install its lazily downloaded binary before CI captures its path.
