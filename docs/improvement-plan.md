...
| Electron/build caches         | Evaluated in [PR #42](https://github.com/Dytschgo/imnota/pull/42); not retained because the same-commit cold/warm experiment did not establish worthwhile savings. See the [measured results](build-cache-experiment.md). Revisit only if download costs materially increase.                           |
| Documentation-only CI routing | Completed in PR #38 and exercised by PR #39; retain conservative classification.                                                                                                                                                                                                                        |
| Stable release duplication    | Implemented in the subsequent stable-release CI cleanup: one quality job alongside platform packaging, with candidate/public-install/promotion gates preserved. Record actual stable elapsed time at the next intended release.                                                                         |
| Slow or flaky tests           | [Profiling found duplicate Mac updater verification](verification-timing.md) in PR/main CI. Require the existing post-package platform-suite case and remove its second invocation. Preserve native, signature, architecture, and release checks; use further traces before changing waits or fixtures. |
...
- The Electron/build-helper archive cache was evaluated and removed after a full three-OS cold/warm pilot. All archives hit on the warm run, but restore overhead and about 523 MB of additional storage did not establish worthwhile savings. Existing dependency caches remain; see the [experiment and limits](build-cache-experiment.md).
- The subsequent [stable-release CI cleanup](stable-release-ci.md) moves shared checks into concurrent quality and retains platform suites and publication gates. It uses classifier fixtures, workflow review, and full PR package validation; actual tag-triggered publication/public-install execution and elapsed-time measurement remain for the next real stable release.
