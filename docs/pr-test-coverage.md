...
The September 22 review additionally classified `CaptureDestinationDialog`, `capture-destination`, `FloatingUpdateControl`, and `WhatsNew`. These use synthetic project/release inputs, callback spies, and jsdom focus/interaction checks; they do not capture a real display, install an update, or read project files. They still run in full quality. The former `CaptureDisplayDialog` and its test/policy entry were removed when direct selection across all displays replaced the monitor chooser; App and native capture checks protect that outcome. Native capture, updater, keyboard navigation, and filesystem coverage stays in the platform suites and packaged walkthroughs. `tests/platform-test-policy.test.ts` protects additive exclusions, the full-suite override, and rejection of duplicate, wildcard, stale, native, or shared paths.
