...
Policy introduced in PR #36. This map describes application-impact PRs and full validation runs; [prose-only PRs](ci-routing.md) use the separately reviewed documentation route. The full suite remains in Linux `quality`. Package jobs run `test:platform`: the same default discovery minus the 53 exact renderer paths in [the policy](../tests/platform-test-policy.json), followed by all script tests. The current inventory collects 126 files in full quality and 73 on each platform, including the policy regression test. Hosted share contracts, the three service jobs, actual packaged walkthroughs, and Windows visual checks remain unchanged. Nightly keeps its full-suite command. [Stable release CI](stable-release-ci.md) runs the full suite once in quality and the platform subset plus all script tests in every package job.
...
The September 22 review additionally classified `CaptureDestinationDialog`, `CaptureDisplayDialog`, `capture-destination`, `FloatingUpdateControl`, and `WhatsNew`. These use synthetic display/project/release inputs, callback spies, and jsdom focus/interaction checks; they do not capture a real display, install an update, or read project files. Their 19 cases still run in full quality. Native capture, updater, keyboard navigation, and filesystem coverage stays in the platform suites and packaged walkthroughs. `tests/platform-test-policy.test.ts` protects additive exclusions, the full-suite override, and rejection of duplicate, wildcard, stale, native, or shared paths.
...
| Unpackaged native walkthrough            | Local `pnpm smoke`; CI uses the packaged walkthrough of the same built application      |
