...
A local Windows native probe confirmed that injected moves need Electron's `leftbuttondown` modifier to report a held button; `button: 'left'` alone still reports zero buttons for a move. The drag driver now supplies that modifier. Local native smoke passed the canvas persistence assertion with this synchronization before stopping later in Sharing settings. That partial run does not replace the required complete packaged CI result.
