...
npm ci
npm test
IMNOTA_SHARE_PUBLIC_ORIGIN=http://localhost:3000 IMNOTA_SHARE_DATA_DIR=./var npm start
```
...
The public and management tokens are separate, domain-separated HMAC-SHA-256 outputs derived from the random 256-bit upload capability and the client-generated UUID. Only their SHA-256 hashes are stored. The client must retain the URL and management token in local share metadata; neither can be recovered from the service database alone.

Limits are 200 title characters, 1 MiB of Markdown, 20 PNGs, 10 MiB per PNG, 25 MiB decoded per bundle, 10,000 pixels per image dimension, a 36 MiB encoded JSON body, and 1–30 days of retention (30 by default). Environment variables can lower or raise the byte and retention limits, but a production change needs matching capacity and abuse review. Upload is one request and has no resumable/chunked protocol. The desktop may cancel it by aborting the HTTP request. Cancellation is an ambiguous transport outcome until receipt recovery confirms whether the server committed it.
...
Expected upload codes are `invalid_token` or `pairing_expired` (401), `pairing_used` or `idempotency_conflict` (409), `recovery_expired` (410), `invalid_request` (400), `payload_too_large` (413), `quota_exceeded` (507), and `rate_limited` (429). Receipt lookup additionally uses `receipt_not_found` (404). The recovery window defaults to 24 hours.
...
There are no public indexes, sitemaps, discovery APIs, analytics, or recipient accounts. Share pages send `X-Robots-Tag: noindex, nofollow`, `Cache-Control: private, no-store`, a restrictive CSP, frame restrictions, no-referrer policy, MIME sniffing protection, and other Helmet defaults.

## Operations

See [Hostinger deployment](docs/hostinger-deployment.md) for the production directory, environment, TLS proxy, backups, restore, cleanup, monitoring, and rollback procedure.
